...
var buscaProducto = function( codigo ) {
   ...
   var win = new Window( 'S', 'Producto', colores.naranja );
   ...
   win.addEventListener("open",function(){
           ...
           peticionHTTP.onload =  function() {
               ...
               if(json.encontrado==1){
                   Utiles.mensajeBurbuja('warning',contenedor, 'CORTESIAS' );
...
function bajasDelProducto( contenedor, producto, win ){
   ...
   btnAceptar.addEventListener("singletap",function(){
       if(jCantidad.input.value==''){
           Utiles.Alerta('Debes capturar la cantidad de producto que se está obsequiando');
           return false;
       }
       ...
       if(jMotivo.input.value==''){
          Utiles.Alerta('Debes capturar el motivo del regalo');
          return false;
       }
...
function RegistrarBajaEnServidor( win, producto, jCantidad, jMotivo ){
    var json = Utiles.obtenerJSON('datos') ;
       ...
       peticionHTTP.onload =  function() {
           ...
           console.log( json );
           win.close();
           Utiles.NotificacionBanner('success','Cortesía registrada',producto.nombre + "[" + jCantidad.input.value + "]");
       };
       var url = params.API + 'producto/cortesias/inventario';
       peticionHTTP.open("POST", url );
       peticionHTTP.setRequestHeader('token',json.datos.api_token);
       peticionHTTP.send({
